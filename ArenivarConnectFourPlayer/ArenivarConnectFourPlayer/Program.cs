...
namespace ArenivarConnectFourPlayer
{
	...
	class MainClass
	{
		...
		{
			// We'll store the JSON data in this string before we can deserialize it into
			// an object
			...
			while ((gameStateData = Console.ReadLine ()) != null && gameStateData != "") {
				// Storing the JSON data from the driver into a GameState object so we can 
				// process it
				...
				// Calling the MaximizeValue function, which will return a Move object that
				// contains the best move according to our algorithm
				Move bestMove = GameUtilities.MaximizeValue (gm, GameUtilities.SearchDepth, 
									GameUtilities.AlphaInitialValue, 
										GameUtilities.BetaInitialValue);
